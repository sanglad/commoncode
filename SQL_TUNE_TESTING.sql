...
:p_from_period -- From GL effective period number (inclusive)
:p_to_period -- To GL effective period number (inclusive)
*/

WITH xla_tbl AS (
    SELECT xlate.source_id_int_1                   AS trx_id,
           ...
           TO_CHAR(xlaah.accounting_date,'MM/DD/YYYY') AS gl_date,
           MIN(CASE WHEN glps.effective_period_num = :p_from_period THEN glps.start_date END)
               OVER ()                            AS from_start_date,
           MAX(CASE WHEN glps.effective_period_num = :p_to_period THEN glps.end_date END)
               OVER ()                            AS to_end_date,
           MIN(CASE WHEN glps.effective_period_num = :p_from_period THEN glps.start_date END)
               OVER () - 1                        AS opening_as_of_date
    ...
    JOIN   gl_ledgers                gll   ON glps.ledger_id     = gll.ledger_id
    JOIN   gl_code_combinations      gcc   ON xlaal.code_combination_id = gcc.code_combination_id
...
period_bounds AS (
    SELECT MIN(from_start_date)    AS from_start_date,
           MAX(to_end_date)        AS to_end_date,
           MIN(opening_as_of_date) AS opening_as_of_date
    FROM   xla_tbl
),
base_trx AS (
    ...
    JOIN   ar_payment_schedules_all ps
           ON ps.customer_trx_id = rct.customer_trx_id
          AND rct.org_id         = ps.org_id
    WHERE  rct.complete_flag = 'Y'
    AND    EXISTS (
               SELECT 1
               FROM   xla_tbl xt
               WHERE  xt.trx_id = rct.customer_trx_id
           )
),
inv_acct AS (
